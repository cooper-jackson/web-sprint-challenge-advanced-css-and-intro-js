...
function get20s(array) {
  ...
  for (let i = 0; i < array.length; i++) {
    // console.log("Task 4: ", array[i][2])
    const year = array[i].years.split(' ');
    if (year[0] >= 1900 && year[0] <= 2000 && year[2] >= 1900 && year[2] <= 2000) {
      get20s_array.push(array[i].name)
    }
  }
  return get20s_array
}
...
function addArtist(array){
    const cooper_jackson = { 
      id: 20,
      name: "Cooper Jackson", 
      years: "1993 - 2021",
      genre: "Web Design", 
      nationality: "TEXAN, YEEEEEHAW",
      bio: "Now, this is a story all about how. My life got flipped-turned upside down. And I'd like to take a minute. Just sit right there. I'll tell you how I became the prince of a town called Bel Air"
    }
    array.push(cooper_jackson)
    return array;
  }
...
function lotsOfArt(array){
  const artists_with_101_paintings = []
  for (let i = 0; i < array.length; i++) {
    if (array[i].paintings > 100) {
      artists_with_101_paintings.push(array[i].name)
    }
  }
  return artists_with_101_paintings
}
